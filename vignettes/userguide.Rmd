...
package: rebook 
vignette: >
  %\VignetteIndexEntry{01. Reusing book content}
...
Most of these ideas were developed in the process for the [Orchestrating single-cell analysis](osca.bioconductor.org) book,
...
To perform this retrieval inside the acceptor chapter, 
...
```{r, results="asis"}
extractCached("test.Rmd", chunk="godzilla-1954", object="godzilla")
```
...
```{r, results="asis"}
extractCached("test.Rmd", chunk="ghidorah-1964", object=c("godzilla", "ghidorah"))
```
...
```{r, results="asis"}
extractCached("test.Rmd", chunk="godzilla-2014", object="godzilla")
```
...
```{r, results="asis"}
extractCached("test.Rmd", chunk="godzilla-2014", object=c("mechagodzilla", "godzilla"))
```
